...
Push all the characters in ch[] mapped to corresponding frequncy freq[] in
priority queue. To create Huffman Tree, pop two nodes from priority queue.
...
Traverse the onlynode in the priority queue for each character in ch[] and store
the Huffman Code. To print all the stored Huffman Code for every character in
ch[].
...
public:
  ...
  bool is_empty() { return this->size == 0 ? true : false; }
};
