...
/**
 * Generates all structurally unique Binary Search Trees (BSTs) for keys from 1
 * to n.
 *
 * <p>
 * This problem is a classic example of dynamic programming and is related to
 * Catalan numbers.
 * The number of unique BSTs for n keys is the n-th Catalan number.
 *
 * <p>
 * Algorithm: The core idea is to iterate through each number `i` from 1 to `n`
 * and consider it
 * as the root of a BST. When `i` is the root, all numbers from `1` to `i-1`
 * must be in the left
 * subtree, and all numbers from `i+1` to `n` must be in the right subtree.
 *
 * <p>
 * We recursively solve the problem for the left and right subtrees. The total
 * number of unique
 * trees is the sum of possibilities for each root `i`.
 *
 * <p>
 * To optimize, we use memoization (a top-down DP approach) to store the list of
 * generated
 * subtrees for a given range of keys (e.g., from `start` to `end`), avoiding
 * redundant
 * computations.
 *
 * <p>
 ...
 * complexity is high
 * because we are constructing all possible trees. The number of trees grows
 * very quickly. Space
