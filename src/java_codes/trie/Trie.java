package java_codes.trie;

import java_codes.trie.utils.TrieNode;
 ...
 * stores a dynamic set
 * of strings. Each node in the Trie represents a single character of a string.
 * A path from the root
 * to a node represents a prefix, and if the node is marked as an end-of-word
 * node, it represents a
 * complete word.
 *
 * <p>
 * This implementation is for lowercase English alphabet characters ('a' through
 * 'z').
 *
 * <p>
 * Time Complexity: Let L be the length of the key (string). - Insertion: O(L) -
 * Search: O(L) -
 * Starts With (prefix search): O(L)
 *
 * <p>
 * Space Complexity: O(ALPHABET_SIZE * L * N) where N is the number of keys in
 * the trie. In the
 * worst case (no shared prefixes), space is the sum of the lengths of all
 * words.
 */
public class Trie {
