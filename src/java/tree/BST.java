 ...
 *
 * <p>Time Complexity (for a balanced tree): - Search: O(log n) - Insertion: O(log n) - Deletion:
 * O(log n)
 *
 * <p>Time Complexity (for a skewed tree - worst case): - All operations: O(n)
 *
 * <p>Space Complexity: O(n) for storing n nodes. The recursion stack for operations is O(h) where h
 * is the height.
 */
public class BST {
