...
// Auxillary Space: O(V).
// Approach: This simply involves BFS approach. That's why time complexity of
// algorithm is O(V+E).
...
void topological_sort(int **graph, int v) {
  ...
  while (!x.is_empty()) {
    ...
    result.enqueue(k);
    for (int i = 0; i < v; i++) {
      // If there exists any edge between the current vertex and any other
      // vertex. Decrease its indegree by 1.
      if (graph[k][i] == 1) {
        indegree[i] -= 1;
        // Check after increasing the indegree that if there exists a node which
        // has 0 indegree, enqueue it.
        if (indegree[i] == 0) {
          x.enqueue(i);
          cout << "X Enqueue: " << i << endl;
        }
    ...
    // If all the nodes are not enqeued or any node is enqueued more than once,
    // then count will not be equal to no of vertices, then, the given graph
    // isn't DAG.
    count++;
  }
  if (count != v) {
