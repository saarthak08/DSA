...
keys in tree. Using Trie, we can search the key in O(M) time. However the
penalty is on Trie storage requirements. So, trie enables string searching in
O(M) time where M is the string length.
...
bool search_in_trie(trie_node *root, string search) {
  ...
  // If all index are not NULL, then at last it is checked whether the last
  // index is marked as end of word or not, if yes then string found, else not
  // found.
  if (root->isEndofWord) {
    return true;
  } else {
...
// Node is checked whether it is empty or not i.e. its all children are NULL or
// not.
bool isEmpty(trie_node *root) {
  for (int i = 0; i < 26; i++) {
    if (root->children[i] != NULL) {
...
trie_node *delete_key(trie_node *root, string toDelete, int index) {
  ...
  // last index is checked if its endOfWord, it is unmarked. // Recursion base
  // case.
  if (index == toDelete.length()) {
    if (root->isEndofWord) {
      root->isEndofWord = false;
    }
    // it is checked whether, it is empty or not. If its empty, then only it is
    // deleted.
    if (isEmpty(root)) {
      delete root;
      root = NULL;
