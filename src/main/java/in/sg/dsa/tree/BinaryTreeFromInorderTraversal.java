...
/**
 * Generates all structurally unique Binary Trees given an in-order traversal sequence.
 *
 * <p>For a sequence from 1 to n, this problem is equivalent to generating all unique Binary Search
 * Trees (BSTs), as the in-order traversal of such a BST is always 1...n. The number of such unique
 * trees is the n-th Catalan number.
 *
 * <p>Algorithm: The core idea is to iterate through each element `inorder[i]` and consider it as
 * the root. When `inorder[i]` is the root, all elements in the in-order traversal before it must
 * form the left subtree, and all elements after it must form the right subtree.
 *
 * <p>We recursively solve the problem for the left and right sub-arrays to generate all possible
 ...
 * because we are constructing all possible trees. Space Complexity: O(n * C_n) to store all the
...
public class BinaryTreeFromInorderTraversal {
  ...
  /**
   * Recursively generates all unique binary trees for the given in-order subarray.
   *
