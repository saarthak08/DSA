...
/*
 * Prim's Algorithm for finding the Minimum Spanning Tree (MST) of a connected, undirected, weighted in.sg.dsa.graph.
 *
 * Explanation:
 * - The in.sg.dsa.graph is represented using an adjacency list.
 * - The algorithm starts from vertex 1 (0-th index in array).
 * - It uses a priority queue (min-in.sg.dsa.heap) to always pick the smallest edge that connects a visited node to an unvisited one.
...
public class Prims {
  ...
  public static void main(String[] args) {
    ...
    while (!queue.isEmpty() && mst.size() != v - 1) {
      ...
      if (!visited[edge.getDest() - 1]) {
        mst.add(edge);
        visited[edge.getDest() - 1] = true;
        for (GraphEdge nextEdge : adjList[edge.getDest() - 1]) {
          if (!visited[nextEdge.getDest() - 1]) {
