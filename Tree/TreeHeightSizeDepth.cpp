...
int main() {
	...
	cout << "Depth of Node 3 :" << endl << depth_of_a_node(root,root->right_child->right_child) << endl;
}
...
void build_tree(Node* root) {
	...
	Node *temp_node;
	if(root!=NULL) {
		cout << "Press \'1\' to enter the left child of " << root->data << ": "<< endl;
		cin >> temp;
		if(temp==1) {
			temp_node = new Node;
		...
		cout << "Press \'1\' to enter the right child of " << root->data << ": "<< endl;
		cin >> temp;
		if(temp==1) {
			temp_node = new Node;
